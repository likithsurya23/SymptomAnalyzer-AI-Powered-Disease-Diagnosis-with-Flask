...
def get_predicted_value(patient_symptoms):
    ...
    return diseases_list.get(disease_index, "Unknown Disease")
...
def predict():
    ...
    if symptoms:
        ...
        # Ensure symptoms are in the dictionary
        missing_symptoms = [s for s in user_symptoms if s not in symptoms_dict]
        if missing_symptoms:
            message = f"Warning: These symptoms are not recognized: {', '.join(missing_symptoms)}"
